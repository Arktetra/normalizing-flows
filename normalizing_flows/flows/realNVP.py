...
class RealNVP(nn.Module):
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
        -------
            Tuple[torch.Tensor, torch.Tensor]: Output sample in latent space, log
    ...
    @torch.no_grad()
    def sample(
        self, shape: torch.Size, z_init: torch.Tensor = None
        ...
        ----
            ...
            z_init (torch.Tensor, optional): An input from the latent space. Defaults to
            None.

        Returns:
        -------
            Tuple[torch.Tensor, torch.Tensor]: Output sample in the data space and the
        ...
        for i in range(self.num_coupling_layers)[::-1]:
            z, log_det_inv = self.layers[i](
                z, log_det_inv, self.masks[i], training=False
            )
    ...
    def training_step(
        self, dataloader: torch.utils.data.DataLoader, optimizer: torch.optim.Optimizer
    ) -> torch.Tensor:
        """Performs a single training step.

        Args:
        ----
            dataloader (torch.utils.data.DataLoader): An instance of the PyTorch
            DataLoader.
    ...
    def validation_step(self, dataloader: torch.utils.data.DataLoader) -> torch.Tensor:
